    ...
    class NodeValueMathDown
    {
            ...
            for (int i = 0; i < values.Length; i++)
            {
                ...
                for (int j = 0; j < nod.props[i].values.Count; j++)
                {
                    ...
                    catch(System.IndexOutOfRangeException ex)
                    {
                        ...
                        nod.props[i].value_editor_down = values[i];
                        return values;
                    }
                }
                nod.props[i].value_editor_down = values[i];
        ...
        public double getNodPropsValueEditor(List<Node_struct> list, Node_struct nod, Propertys_struct proppppp)
        {
            ...
            иззаошибки:
                ...
                if (getValues_editors(см, nod, list)[0]==-1)
                    goto иззаошибки;
                value = proppppp.value_editor_down;//nod.props[i].value_editor;
            }
            else
            {
                value = proppppp.value_editor_down = proppppp.values[0];//nod.props[i].values[0];
            }
            return value;
        }
    }
